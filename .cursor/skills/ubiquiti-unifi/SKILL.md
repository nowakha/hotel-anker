...
1. **Read-only first** â€” inventory before any change.
2. **Never** `factory-reset.sh`, never reboot the UDM unless the user explicitly asks.
3. **Never** write MongoDB `ace` for portal/WLAN branding â€” use UniFi UI / official API only.
4. **Never** paste `x_api_token`, device SSH passwords, or PSKs into markdown docs â€” secrets YAML only.
...
7. Prefer **Insights â†’ Hotspot â†’ Landing Page** for branding. Exception: if the portal SPA is missing (`index.html` 404 / empty body), restore the stock package with `scripts/repair_unifi_hotspot_portal.py` (no mongo writes).
...
UI admin may differ from SSH root â€” if API login returns 403, do not brute-force; ask for UI credentials or use documented UI steps.

## Site map (SSID â†” VLAN)
...
- Tri-band single SSIDs (2.4/5/6) â€” **do not** recreate legacy `HotelAnker_5G` as primary.
- AnkerPI01/02 live on **Administration** (`.1.x`), PSK `HeimatSchutz` â€” not Bar VLAN `.2.x`.
...
From Windows, use Paramiko (password from secrets) â€” OpenSSH password prompts are awkward in agents.
...
3. After user visual OK â†’ UniFi Network â†’ **Insights â†’ Hotspot â†’ Landing Page**
4. Map fields from [`NETWORK_UNIFI.md`](../../../WerbeLEDbox-CountDown/docs/NETWORK_UNIFI.md) Â§ Landing Page
