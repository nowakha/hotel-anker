...
def draw_cross_section(w: int = 1400, h: int = 900) -> Image.Image:
    ...
    d.text(
        (40, 64),
        f"Profil-Tiefe ≈ {PROFILE_DEPTH_MM:.0f} mm (Kendu FAQ «spessore»; Stirnbreite separat {PROFILE_FACE_W_MM:.0f} mm gemessen)",
        fill=(70, 70, 80),
        font=font(18),
    )

    # Draw a horizontal section through left profile + face
    # Coordinates: left = front (viewer), right = back
    front_x = 180
    depth_px = 720
    scale_z = depth_px / PROFILE_DEPTH_MM
    top = 180
    bot = 720
    mid = (top + bot) // 2

    # Aluminium profile U-ish: front lip, outer wall, back
    alu = (170, 178, 188)
    d.rectangle([front_x - 40, top, front_x + depth_px + 40, bot], fill=(230, 232, 236), outline=(140, 145, 155), width=2)

    # Front face rim + keder groove
    lip_w = int(PROFILE_FACE_W_MM * 0.6)  # schematic
    ...
    # Air / diffusion gap
    led_x = front_x + int(PROFILE_DEPTH_MM * 0.55 * scale_z)
    d.rectangle([front_x + 30, top + 40, led_x - 8, bot - 40], fill=(255, 252, 230))
    d.text(((front_x + 30 + led_x) // 2 - 40, mid - 40), "Luft /\nDiffusion", fill=(140, 120, 40), font=font(16))
    ...
    # White reflector / back
    back_x = front_x + depth_px
    ...
    # Controllers at bottom of section
    ...
    # Depth dimension
    y_dim = bot + 40
    d.line([front_x, y_dim, back_x, y_dim], fill=(40, 40, 48), width=2)
    d.line([front_x, y_dim - 8, front_x, y_dim + 8], fill=(40, 40, 48), width=2)
    d.line([back_x, y_dim - 8, back_x, y_dim + 8], fill=(40, 40, 48), width=2)
    d.text(
        ((front_x + back_x) // 2 - 80, y_dim + 12),
        f"≈ {PROFILE_DEPTH_MM:.0f} mm Profil-Tiefe (Z)",
        fill=(40, 40, 48),
        font=font(18, True),
    )
    ...
    d.text(
        (40, h - 70),
        f"Keder-Nut typ. {KEDER_GROOVE_W_MM:.0f}×{KEDER_GROOVE_D_MM:.0f} mm (Industrie-Annahme) · Foto 02: schmale Nut an Innenlippe",
        fill=(90, 90, 100),
        font=font(16),
    )
    return im
